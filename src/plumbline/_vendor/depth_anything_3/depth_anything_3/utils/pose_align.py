...
import numpy as np
import torch
# plumbline vendor: `evo` import made lazy — it's only needed for multi-view
# pose alignment (PosePath3D), never on the mono-depth path. Imported on demand
# inside the functions that use it so base install doesn't require the evo stack.
...
def _umeyama_sim3_from_paths(pose_ref, pose_est):
    from evo.core.trajectory import PosePath3D  # plumbline vendor: lazy
