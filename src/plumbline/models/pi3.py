...
class Pi3Adapter(Model):
    ...
    capabilities = ModelCapabilities(
        ...
        min_views=2,
        # Upstream doesn't document a hard cap. π³ is feed-forward and
        # permutation-equivariant, so it ingests a whole clip in one pass —
        # trajectory-pose eval (DAGE Table 4) needs the full sequence (the
        # metric requires pred and GT to be the same length), and Sintel's
        # dynamic clips run to ~50 frames. 64 covers them; the runner's OOM
        # fallback catches anything a given card can't hold (a 50-frame clip
        # at fp16 is near the edge of an 11GB card).
        max_views=64,
...
def _ensure_pi3_on_path() -> None:
    """Add the vendored ``pi3`` package root to sys.path so `from pi3.models…` resolves.

    Pi3 is vendored under ``plumbline/_vendor/pi3`` (BSD-3-Clause — permissive,
    redistributable; see THIRD_PARTY_NOTICES.md). Its internal imports are
    absolute (``from pi3.models.pi3 import Pi3``), so the vendor root — the
    directory containing the ``pi3/`` package — must be importable. ``$PI3_ROOT``
    overrides for a dev checkout.
    """
    root = os.environ.get("PI3_ROOT")
    if not root:
        root = os.path.join(os.path.dirname(os.path.dirname(__file__)), "_vendor", "pi3")
