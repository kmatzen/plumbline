...
| `vggt-eth3d-courtyard-chamfer` | VGGT on ETH3D courtyard, 8-view | `chamfer` | _n/a_ | **6.84** | n/a | RTX 3090 Ti, 2026-04-19, **ICP alignment**. F-score@0.05=**66.95%**, precision=68.76%, recall=81.59%. Huge jump vs camera-centres Umeyama (F=0.12%) — ICP is the paper's protocol. Chamfer aggregate still elevated because it's dominated by far-outlier hallucinations; thresholded metrics (F/precision/recall) are the paper-comparable signal. |
| `da3-eth3d-courtyard-chamfer` | DA3 Large-1.1 on ETH3D courtyard, 8-view | `chamfer` | _n/a_ | **7.14** | n/a | RTX 3090 Ti, 2026-04-19, **ICP alignment**. F-score@0.05=**61.17%**, precision=**84.13%**, recall=76.83%. Direct A/B with VGGT (F=66.95%, prec=68.76%, rec=81.59%) on same slice: DA3 is more conservative — higher precision, lower recall; VGGT denser, more hallucinations. Both models work under ICP — confirms protocol dominance. |
...
| `moge-vitl-nyuv2` | MoGe-1 ViT-L on NYUv2 Eigen (Table 3) | `abs_rel` | 0.0297 | **0.0342** | ±20% | ⚠️ **adapter validated end-to-end, paper-alignment gap**. RTX 3090 Ti, 2026-04-19. δ₁: paper 0.986, observed 0.9824. Gap (+15% AbsRel) is the paper's ROE alignment vs plumbline's scale_shift LSQ; not an adapter bug. ROE is v0.2 parking-lot. |
| `moge2-vitl-nyuv2` | MoGe-**2** ViT-L on NYUv2 Eigen | `abs_rel` | _n/a_ | **0.0342** | ±20% | δ₁=0.9831. v1-vs-v2 A/B on identical eval: indistinguishable under scale_shift (both 0.0342). The v1-vs-v2 architectural improvement requires metric-eval (`scale_alignment: none`) to surface — see `moge2-vitl-nyuv2-metric`. |
...
| `moge-vitl-diode-indoor` | MoGe-1 ViT-L on DIODE val-indoor | `abs_rel` | 0.0313 | **0.0584** | ±20% | RTX 3090 Ti, 2026-04-19, 325 indoor samples. δ₁=0.9525. Paper is combined val (325+446=771 samples) so indoor-only isn't a direct A/B; still ~2x off which likely combines "indoor harder than outdoor" + ROE-vs-scale_shift. Beats DA-V2-small on the same slice (0.0722). |
