...
Sintel + Bonn lineage cells (also MonST3R Table 3 single-frame, same paper) landed alongside the KITTI close but as **ℹ️ informational** — both structurally faithful but off-paper >5 %:
- **MonST3R-Sintel** (`sintel_dust3r_lineage`, 14 dynamic-scene clips, max_depth=70 sky-mask, per-frame median): ℹ️ AbsRel **0.3772** vs paper 0.345 _(9.3 % off, worse; companion δ₁ 0.562 vs 0.565 is within 0.5 %)_. Per-scene breakdown identifies `temple_2` (mean 0.935, max 7.87) as the dominant outlier — a known fragility of the demo view-duplicate single-frame trick on textureless / heavily-occluded scenes; without `temple_2` the aggregate is ~0.330, within tolerance. Not a paper-match cell; the lineage protocol + reproduction YAML are committed for the next adapter-fidelity sprint (suspected fix: replace the MASt3R-PairViewer N=2 path with an explicit symmetric-pair averaging à la MonST3R's `eval_mono_depth`'s `pred1.pts3d.mean(dim=0)`).
- **MonST3R-Bonn** (`bonn_dust3r_lineage_single`, 5 sequences × all RGB frames, per-frame median, max_depth=70): ℹ️ AbsRel **0.0656** vs paper 0.076 _(13.7 % off, better)_. Same root-cause hypothesis as Sintel — re-scoring on MonST3R's exact `rgb_110/depth_110` subset (frames [30:140] × 5 seqs = 550 samples) lands at AbsRel 0.0635, still ~16 % off-paper-better, ruling out frame-subset as the culprit. Companion δ₁ 0.959 vs 0.939 (also off, better). Documented protocol delta, follow-up gated on the symmetric-pair averaging fix above.
