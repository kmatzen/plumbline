...
| **UniK3D Large** _(metric, no align)_ | ✅ **0.0749** vs 0.074 _(δ₁ 0.9656 vs 0.965; RMSE 0.2632 vs 0.259; Table 18 zero-shot NYUv2 metric columns; 654/654 on the 1080 Ti, 2026-06-11. First UniK3D cell — default inference bounds reproduce the paper out of the box.)_ | — | ℹ️ **0.1509** vs 0.161 _(Indoor val 325/325, [0.01,25] cap; UniK3D Table 22; 6.3% under — δ₁ 0.754 / RMSE 0.718 both better; 2026-06-13. Metric native, distinct from the MoGe-eval affine DIODE column.)_ | ℹ️ **0.1544** vs 0.236 _(native-depth 454/454, [0.01,50] cap; UniK3D Table 21; off-paper **better** — δ₁ 0.814 / RMSE 1.07 — frame-set/res protocol diff vs UniK3D's HDF5 eval; 2026-06-11. Metric native, distinct from the MoGe-eval affine ETH3D column.)_ | — | — | — |
...
| **Depth Pro** | ℹ️ δ₁ **0.9347** _(no NYU row in paper)_ | — | ℹ️ ETH3D δ₁ **0.3648** vs **0.415** _(12.1% under, GT focal; self-focal 0.3339. RAN 2026-06-11 on 454/454 — exact Table 16 manifest. Bimodal: close indoor scenes match paper (kicker 0.90, office 0.92), 3 far-range outdoor scenes collapse to ≈0 — Depth Pro under-scales far metric depth ~3.7×. NOT tuned; [handoff](docs/ETH3D_DEPTH_PRO_TABLE1_HANDOFF.md))_ | ✅ Booster **0.4878** / **0.466** _(Table 1 match)_ | — | — | ℹ️ iBims **0.8458** _(info)_ · ✅ **Sun-RGBD 0.8682 / 0.890** _(Table 1 MATCH, 2.4% off; resolved 2026-06-01 — native + GT focal via `depth-pro-sun-rgbd-native`, full 5050/5050 official run. See [page](docs/blocked/DEPTH_PRO_SUN_RGBD_TABLE1.md))_ · Table 1 🔒 blocked: [Sintel](docs/blocked/DEPTH_PRO_SINTEL_TABLE1.md) · Midd / NuScenes **loaders removed pre-release** (no verified anchor — [BLOCKED.md](docs/BLOCKED.md)) |
| **MASt3R** (N-view post-2026-04-27) | — | — | — | 2-view pose sweep | — | ✅ AUC@30 **0.8581** vs 0.818 _(faithful `sparse_ga` matching path, full 410-sample re-run 2026-06-03 on the 1080 Ti; +4.9 %, MATCH; companions RRA@15 0.9697 / RTA@15 0.9308 also MATCH). Supersedes the legacy `dust3r_ga` 0.7960 (−2.7 %) which matched on the **wrong** pose method._ See [MASt3R pose-fidelity note](#mast3r-pose-the-matching-path-2026-06-02). | — |
...
| **CUT3R** _(video + unordered)_ | ✅ **0.0858** vs 0.086 _(plumbline-native `cut3r-nyuv2-prepared`: `median_lineage` alignment on CUT3R's exact prepared NYU set via the `nyu-cut3r-eval` loader; 2026-06-15. The strict `cut3r-nyuv2` cell reads ℹ️ 0.0522 — documented protocol delta.)_ | ✅ **0.0953** vs 0.092 _(plumbline-native `cut3r-kitti-lineage`: `median_lineage` on `kitti_dust3r_lineage`, 1269/1269; 2026-06-16. The strict `cut3r-kitti` cell reads ℹ️ 0.0858.)_ | — | — | — | ℹ️ recurrent/online — handles ordered video & unordered sets | — |
...
| **DUSt3R** _(origin paper, single-frame F(I,I))_ | ✅ **0.0637** vs 0.0650 _(−2.0%; RESOLVED 2026-06-18 — DUSt3R's own Table 2 uses the classical **Eigen-2014 crop + ratio-of-medians** (`nyu_dust3r_table2`), which the later lineage drops; the crop was the +19.5%. δ₁ 0.9456 vs 0.9402. See D28 RESOLVED)_ | ✅ **0.1049** vs 0.1074 _(2.3% off; Table 2 KITTI, view-duplicate adapter v1.1, `kitti_dust3r_lineage` protocol; full 1269-frame / 13-drive gathered set; companion δ₁ 0.8661 vs 0.8660 (≈exact; paper δ₁ corrected 0.8600→0.8660 per PDF re-verify 2026-05-28); 2026-05-28)_ | — | — | — | ✅ AUC@30 **0.7893** vs 0.772 _(2.2 % over; MASt3R Table 3 DUSt3R row; target corrected 0.774→0.772 per PDF re-verify 2026-05-28)_ | — |
...
- **DUSt3R-Bonn** (`bonn_lineage_110_single`): ✅ AbsRel **0.1384** vs **0.141** _(−1.8 %, MATCH; δ₁ 0.8331 vs 0.825)_ — **RESOLVED 2026-06-18**, re-targeted from DUSt3R's own Table 2 (0.0808) to **MonST3R Table 3's DUSt3R baseline (0.141)**, which is DUSt3R single-frame depth scored on exactly the `rgb_110` 5-sequence set plumbline evaluates. DUSt3R's own 0.0808 used an unspecified, unrecoverable 2023 Bonn selection (no eval code shipped); MonST3R's hard-5 set (balloon2/crowd2/crowd3/person_tracking2/synchronous) is a 2024 choice from `eval_metadata.py`. A per-sequence probe ruled out every recipe lever (cap, estimator, per-seq vs per-frame scale) — the gap was purely the sequence set, and DUSt3R reproduces the paper on the simpler seqs (balloon2 0.085 ≈ 0.0808). The earlier "DUSt3R is a static model, fails on dynamic Bonn" claim is **retracted** (DUSt3R's own paper reports 0.0808 on Bonn → the model handles it). See `docs/DISCREPANCIES.md` D28 (RESOLVED).
...
CUT3R Table 2 video-depth (per-sequence scale) Bonn AbsRel **0.078**
(verified_pdf) now reproduces **plumbline-native**: ✅ `cut3r-bonn-110`
= **0.0768** vs 0.078 (−1.5 %, 5/5 seqs; 2026-06-16). The exact set is
the 5 Table-2 sequences × `rgb_110` = `sorted(frames)[30:140]` (the new
bonn-loader `frame_start=30`, per MonST3R's `prepare_bonn.py` — NOT
first-110, which gives a catastrophic ~0.42), and `scale_weiszfeld`
(CUT3R's video `--align scale`, robust Weiszfeld IRLS scale-only). On
the right frame set the alignment is secondary (median 0.0770 /
median_lineage 0.0774 / weiszfeld 0.0768 all match). The earlier
`cut3r-bonn` cell (all-seqs / 60-frame, median) stays ℹ️ **0.0536** — a
different selection, not a paper-match. MonST3R also reports Bonn
(0.067 w/ flow GA); a faithful MonST3R-video cell awaits the flow-path
follow-up.

### Paper-match count

**38 ✅ mono-depth cells + 7 ✅ pose cells = 45 total** with `source_confidence: verified_pdf`:
...
- **DUSt3R indoor single-frame (2, RESOLVED 2026-06-18): NYU** (0.0637 vs 0.065, −2.0%) + **Bonn** (0.1384 vs 0.141, −1.8%). Both were off-paper "info" (NYU +19.5%, Bonn +65.5%) under wrong recipes; resolved on anima (official runs). NYU: DUSt3R's own Table 2 uses the classical **Eigen-2014 crop + ratio-of-medians** (`nyu_dust3r_table2` protocol), which the *later* lineage (`nyu_dust3r_lineage`) drops — the crop is the +19.5% (MonST3R Table 3 independently re-scores DUSt3R no-crop at 0.080 ≈ plumbline's 0.0777). Bonn: re-targeted to **MonST3R Table 3's DUSt3R baseline (0.141)** on the `rgb_110` 5-seq set plumbline actually evaluates — DUSt3R's own Table 2 (0.0808) used an unrecoverable 2023 selection (no eval code shipped). The "DUSt3R is a static model, fails on dynamic Bonn" explanation is retracted (DUSt3R's own paper reports 0.0808 on Bonn). See D28 (RESOLVED).
...
matching, which is exactly why the **legacy CO3Dv2 ✅ (0.796) passed on the wrong
method**. The **full 410-sample `sparse_ga` re-run (2026-06-03, 1080 Ti, ~6 h)
re-validates the cell on the faithful method: mAA@30 = 0.8581 vs paper 0.818
(+4.9 %, MATCH)**, +6.2 pt over the legacy `dust3r_ga` 0.796, with companion
RRA@15 0.9697 / RTA@15 0.9308 also matching. This is the number in the matrix
row above; the old 0.796 is retained only as the "right number, wrong method"
footnote. The RealEstate10K cells stay **off-paper** (subset-inflated, like
DUSt3R's +8.5% there) — informational, not new ✅ cells;
`vggt-realestate10k-pose` stays blocked (bf16 → needs an H100). See
`docs/SOURCE_AUDIT.md` and the `mast3r-realestate10k-pose` gpu_queue entry.
...
- **CUT3R NYU + KITTI + Bonn** (D24, 2026-05-25): NYU 0.08595 / paper 0.086; KITTI 0.09219 / 0.092 (Table 1); Bonn 0.07661 / 0.078 (Table 2, per-seq scale) — all within 0.06–1.8 % of paper, the apples-to-apples exact-set reproductions. Via CUT3R's `eval/monodepth` + `eval/video_depth --align scale`. These confirm the model + weights; plumbline's standard-loader cells (matrix) read off-paper purely because they use plumbline's loaders, not CUT3R's prepared `.npy` sets (the `nyu_dust3r_lineage` protocol explicitly does *not* reproduce CUT3R — only MonST3R). **Inference is byte-verified faithful** (2026-06-14, anima): plumbline's `_build_views` preprocessing is **byte-identical** (`max|Δ|=0`) to CUT3R's own `load_images_for_eval`, and the adapter calls CUT3R's own `inference()` with the same z-channel depth extraction — so the gap is **100 % eval-set/GT**, not the adapter (same shape as DUSt3R's D28 byte-verification; it's also why plumbline's lineage run reads *better*, 0.0777, than CUT3R's own 0.08595 — easier test set, identical model). The end-to-end `cut3r-nyuv2-lineage` cell (0.07770, 654/654) confirms the lineage *protocol* is not the missing piece. **DONE 2026-06-15/16 — all three are now plumbline-native ✅ matrix cells** (`cut3r-nyuv2-prepared` 0.0858, `cut3r-kitti-lineage` 0.0953, `cut3r-bonn-110` 0.0768). The gap was the per-frame median *estimator* (plumbline `median` = median-of-ratios; CUT3R = ratio-of-medians → new `median_lineage` mode) and, for Bonn, the video `--align scale` estimator (`scale_weiszfeld`) + the `rgb_110`=[30:140] frame set. PAPER-SPECIFIC: MonST3R-NYU stays on `median` (median_lineage makes it *worse*). See `docs/DISCREPANCIES.md` D24.
...
These failure modes are tracked as known traps in `docs/ARCHITECTURE.md` §10.
