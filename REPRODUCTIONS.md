...
- **MonST3R-Sintel** (`sintel_dust3r_lineage`, 14 dynamic-scene clips, max_depth=70 sky-mask, per-frame median): ℹ️ AbsRel **0.3726** vs paper 0.345 _(8.0 % off, worse; companion δ₁ 0.567 vs 0.565 within 0.4 %)_. Per-scene breakdown identifies `temple_2` (mean 0.93, max 7.87) as the dominant outlier — a known fragility of the view-duplicate single-frame trick on textureless / heavily-occluded scenes; without `temple_2` the aggregate is ~0.32, within tolerance.
- **MonST3R-Bonn** (`bonn_dust3r_lineage_single`, 5 sequences × all RGB frames, per-frame median, max_depth=70): ℹ️ AbsRel **0.0654** vs paper 0.076 _(14.0 % off, better; companion δ₁ 0.960 vs 0.939, also off, better)_. Re-scoring on MonST3R's exact `rgb_110/depth_110` subset (frames [30:140] × 5 seqs = 550 samples) lands at AbsRel 0.0635, ruling out frame-subset as the culprit.

**Adapter v1.0 → v1.1 (2026-05-26, eval-mono-depth-avg null-result):** the suspected single-frame fix — averaging the two symmetric pair predictions (`pred1.pts3d.mean(dim=0)`, MonST3R `eval_mono_depth` shape) instead of routing through the MASt3R-shared PairViewer — was implemented and re-run across all four cells. **Result: all four cells moved by <0.005 AbsRel**, ruling out avg-pred as the cause of the Sintel/Bonn deltas. The v1.1 path is still preserved (it matches MonST3R's upstream eval code verbatim, making the adapter strictly more faithful), but the off-paper deltas trace to a deeper source. Next-best hypotheses: (a) Sintel temple_2 is a MonST3R model-limitation, not adapter; (b) Bonn's off-paper-*better* shape matches the D24 family — likely a GT-side / frame-selection / max_depth subtlety distinct from `prepare_bonn.py [30:140]`. Both deferred until a fresh single-record diff against MonST3R's own `launch.py --mode=eval_depth` output on one sample.
