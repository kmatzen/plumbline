...
| `moge-vitl-diode-indoor` | MoGe-1 ViT-L on DIODE val-indoor | `abs_rel` | 0.0313 | **0.0465** | ±20% | RTX 3090 Ti, 2026-04-19, 325 indoor samples, **ROE alignment**. δ₁=0.9505. Paper is combined val (325+446=771); indoor-only still ~49% over paper. ROE closed 20% from plain-LSQ 0.0584. **Note: outdoor is HARDER than indoor** — see `moge-vitl-diode-both` (combined AbsRel 0.1993). Indoor-only is the closer-to-paper slice. |
| `moge-vitl-diode-both` | MoGe-1 ViT-L on DIODE combined val | `abs_rel` | 0.0313 | **0.1993** | ±15% | RTX 3090 Ti, 2026-04-19, 771 samples, ROE. δ₁=0.8544. 6.4× over paper — gap almost certainly protocol: our `depth_clip: [0.001, 50]` on DIODE outdoor (300 m range) probably wrong. Paper likely clips to 30-80 m and/or masks sky pixels. Indoor-only run is closer to paper — treat indoor as primary until outdoor protocol is nailed. |
