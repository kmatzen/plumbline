...
| `vggt-eth3d-courtyard-chamfer` | VGGT on ETH3D courtyard, 8-view | `chamfer` | _n/a_ | **7.71** | n/a | ⚠️ **adapter+chamfer path validated end-to-end; numbers look off**. RTX 3090 Ti, 2026-04-19. F-score@0.05=0.12%, precision=0.09%, recall=0.22%. Much worse than VGGT paper Table 3's full-dataset mean — likely the camera-centres Umeyama is the wrong alignment for this protocol, or courtyard is a particularly hard single scene. Do not cite these against paper tables. Investigate in v0.2. |
| `da3-eth3d-courtyard-chamfer` | DA3 Large-1.1 on ETH3D courtyard, 8-view | `chamfer` | _n/a_ | **7.99** | n/a | RTX 3090 Ti, 2026-04-19. F-score@0.05=0.12%, precision=0.09%, recall=0.25%. Lands within 4% of VGGT (7.71) on the same protocol → ~8m chamfer is a **protocol effect** (camera-centres Umeyama ≠ paper's ICP), not model-specific. Also validated the runner's depth→point-map back-projection fallback since DA3 doesn't emit a dense point_map. |
